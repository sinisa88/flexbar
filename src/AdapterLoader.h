...
typedef std::pair< SequencingRead<seqan::CharString, seqan::CharString>*, unsigned long > TAdapter;
...
public:
	...
	void printAdapters(std::string adapterName){
		using namespace std;
		
		const unsigned int maxSpaceLen = 23;
		
		cout << adapterName << ":" << string(maxSpaceLen - 8, ' ') << "Sequence:" << endl;
		...
		for(unsigned int i=0; i < adapters.size(); ++i){
			...
			cout << seqTag << whiteSpace << adapters.at(i).first->getSequence() << endl;
		}
		cout << endl;
	}
